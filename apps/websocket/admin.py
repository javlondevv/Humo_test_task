...
class NotificationTypeFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        return [(type_info['type'], f"{type_info['type'].title()} ({type_info['count']})") 
...
class NotificationStatusFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        status_counts = Notification.objects.values('status').annotate(
            count=Count('status')
        ).order_by('type')
...
class NotificationAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'type', 'title', 'recipient_link', 'sender_link',
        'status', 'priority', 'created_at', 'sent_at', 'read_at'
    ]
    ...
    fieldsets = (
        (None, {
            'fields': ('id', 'type', 'status', 'priority')
        }),
        (_('Content'), {
    ...
    actions = [
        'mark_as_sent', 'mark_as_read', 'mark_as_failed',
        'resend_notifications', 'delete_old_notifications'
    ]
    ...
    def mark_as_sent(self, request, queryset):
        ...
        self.message_user(
            request,
            f'{updated} notification(s) were successfully marked as sent.'
        )
    ...
    def mark_as_read(self, request, queryset):
        ...
        self.message_user(
            request,
            f'{updated} notification(s) were successfully marked as read.'
        )
    ...
    def mark_as_failed(self, request, queryset):
        ...
        self.message_user(
            request,
            f'{updated} notification(s) were successfully marked as failed.'
        )
    ...
    def resend_notifications(self, request, queryset):
        ...
        self.message_user(
            request,
            f'{updated} failed notification(s) were queued for resending.'
        )
