...
from apps.utils.constants import (ORDER_STATUSES, WS_MESSAGE_STRUCTURE,
                                  WS_MESSAGE_TYPES)
...
class WebSocketMessageBuilder:
    ...
    @staticmethod
    def create_message(
        message_type: str, payload: Dict[str, Any], timestamp: Optional[datetime] = None
    ) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def create_order_message(event: str, order_id: int, **kwargs) -> Dict[str, Any]:
        """
        ...
        return WebSocketMessageBuilder.create_message(
            message_type=event, payload=payload
        )
...
class WebSocketNotifier:
    ...
    def notify_user(self, user_id: int, message: Dict[str, Any]) -> None:
        ...
        async_to_sync(self.channel_layer.group_send)(
            group_name, {"type": "notify", "message": message}
        )
    ...
    def notify_workers_by_gender(self, gender: str, message: Dict[str, Any]) -> None:
        ...
        worker_ids = list(
            User.objects.filter(role=User.Role.WORKER, gender=gender).values_list(
                "id", flat=True
            )
        )
    ...
    def notify_payment_processed(self, order, success: bool) -> None:
        ...
        """
        event_type = (
            WS_MESSAGE_TYPES["PAYMENT_SUCCESS"]
            if success
            else WS_MESSAGE_TYPES["PAYMENT_FAILED"]
        )

        message = WebSocketMessageBuilder.create_order_message(
            event=event_type, order_id=order.id, status=order.status
        )
