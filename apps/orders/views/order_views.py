...
class OrderCreateView(CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            logger.warning(f"User {self.request.user.id} attempted to create order without permission")
        ...
        try:
            ...
            if not new_status and worker_id:
                ...
                try:
                    ...
                    logger.info(f"Worker {worker.username} assigned to order {order.id}")
            ...
            if worker_id and updated_order.status == Order.Status.PAID:
                ...
                try:
                    ...
                    logger.info(f"Worker {worker.username} assigned to order {updated_order.id}")
                except User.DoesNotExist:
                    return Response(
                        {'detail': 'Worker not found.'},
                        status=status.HTTP_400_BAD_REQUEST
