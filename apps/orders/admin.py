...
class OrderAdmin(ModelAdmin):
    ...
    readonly_fields = ["id", "created_at", "updated_at", "paid_at", "completed_at"]
    ...
    def mark_as_paid(self, request, queryset):
        ...
        self.message_user(
            request, f"{updated} order(s) were successfully marked as paid."
        )
    ...
    def mark_as_in_progress(self, request, queryset):
        ...
        self.message_user(
            request, f"{updated} order(s) were successfully marked as in progress."
        )
    ...
    def mark_as_completed(self, request, queryset):
        ...
        self.message_user(
            request, f"{updated} order(s) were successfully marked as completed."
        )
    ...
    def mark_as_canceled(self, request, queryset):
        ...
        self.message_user(
            request, f"{updated} order(s) were successfully marked as canceled."
        )
    ...
    def assign_workers(self, request, queryset):
        ...
        orders_to_assign = queryset.filter(
            status=Order.Status.PAID, worker__isnull=True
        )

        assigned_count = 0
        for order in orders_to_assign:
            available_worker = User.objects.filter(
                role=User.Role.WORKER, gender=order.client.gender, is_active=True
            ).first()
        ...
        self.message_user(
            request, f"{assigned_count} order(s) were successfully assigned to workers."
        )
    ...
    def export_orders(self, request, queryset):
        ...
        self.message_user(request, f"{queryset.count()} order(s) selected for export.")
