...
class OrderService:
    ...
    @staticmethod
    def create_order(
        client: User, service_name: str, price: int, description: str = "", **kwargs
    ) -> Order:
        """
        Create a new order.
    ...
    def get_order_by_id(order_id: int, user: User) -> Order:
        ...
        try:
            order = (
                Order.objects.select_related("client", "worker")
                .only(
                    "id",
                    "service_name",
                    "description",
    ...
    def update_order_status(
        order: Order, new_status: str, user: User, **kwargs
    ) -> Order:
        """
    ...
    def complete_order(order: Order, user: User) -> bool:
        ...
        if order.worker != user:
            raise InsufficientPermissionsError(
                "Only assigned worker can complete the order."
            )
    ...
    def cancel_order(order: Order, user: User, reason: str = "") -> bool:
        ...
        if success:
            logger.info(
                f"Order {order.id} canceled by user {user.id}. Reason: {reason}"
            )
    ...
    def process_payment(order: Order, success: bool = True, **kwargs) -> Order:
        ...
        try:
            with transaction.atomic():
                ...
                logger.info(
                    f"Payment processed for order {order.id}. Success: {success}"
                )
    ...
    def refund_payment(order: Order, user: User, reason: str = "") -> bool:
        ...
        try:
            with transaction.atomic():
                ...
                logger.info(
                    f"Refund processed for order {order.id} by user {user.id}. Reason: {reason}"
                )
