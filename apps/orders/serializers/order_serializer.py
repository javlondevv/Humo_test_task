...
from rest_framework.serializers import ModelSerializer, ValidationError, Serializer, CharField, ChoiceField, \
    IntegerField, ReadOnlyField, DateField
...
class OrderStatusUpdateSerializer(Serializer):
    ...
    worker_id = IntegerField(
        required=False,
        allow_null=True,
        help_text='ID of worker to assign (optional)'
    )
...
class OrderDetailSerializer(ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'client', 'worker', 'created_at', 'updated_at', 'paid_at', 'completed_at',
            'can_be_paid', 'can_be_assigned', 'can_be_started', 'can_be_completed', 'can_be_canceled',
            'is_pending', 'is_paid', 'is_in_progress', 'is_completed', 'is_canceled'
        ]
        read_only_fields = [
            'id', 'status', 'client', 'worker', 'created_at', 'updated_at', 'paid_at', 'completed_at'
        ]
...
class OrderListSerializer(ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = [
            'id', 'service_name', 'price', 'status', 'status_display',
            'client_username', 'worker_username', 'created_at'
        ]
...
class OrderPaymentSerializer(Serializer):
    ...
    amount = IntegerField(
        help_text='Payment amount (should match order price)'
    )
...
class OrderActionSerializer(Serializer):
    ...
    def validate(self, attrs):
        ...
        if action == 'cancel' and not reason.strip():
            raise ValidationError({
                'reason': 'Reason is required for cancellation.'
            })
