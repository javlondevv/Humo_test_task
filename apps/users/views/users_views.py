...
from apps.users.serializers.users_serializers import (
    UserCreateSerializer, UserDetailSerializer, UserListSerializer,
    UserLoginSerializer, UserPasswordChangeSerializer, UserUpdateSerializer)
...
class UserRegisterView(CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            logger.warning(
                f"User {self.request.user.id} attempted to create user without permission"
            )
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            updated_user = UserService.update_user(
                user=user, requesting_user=request.user, **serializer.validated_data
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == "activate":
                ...
                logger.info(
                    f"User {target_user.id} role updated to {new_role} by admin {request.user.id}"
                )
...
class WorkerListView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        try:
            queryset = (
                User.objects.filter(
                    role=User.Role.WORKER, gender=gender, is_active=True
                )
                .select_related()
                .only(
                    "id",
